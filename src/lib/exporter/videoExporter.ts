			...
			await streamingDecoder.decodeAll(
				...
				async (videoFrame, _exportTimestampUs, sourceTimestampMs) => {
					...
					try {
						...
						// Create VideoFrame directly from the 2D composite canvas.
						// This avoids the expensive getImageData() CPU readback and
						// lets the browser transfer the canvas bitmap to the encoder
						// via GPU-side texture sharing when available.
						//
						// The compositeCanvas is a standard 2D canvas (not WebGL),
						// so it is not affected by the Linux EGL/Ozone green-frame
						// bug that only occurs when drawImage() reads from a WebGL
						// canvas. The WebGL readPixels workaround in frameRenderer
						// already handles that step before compositing.
